...
def get_energy_error(file_path, model_path, flashmd_model_path):
    ...
    calculator = MetatensorCalculator(
        model_path, extensions_directory="extensions_pet_mad/", device=device
    )
    flashmd_model = load_atomistic_model(flashmd_model_path)

    delta_q_key = [
        k for k in flashmd_model.capabilities().outputs.keys() if "mtt::delta_" in k
    ][0]
    ...
    plt.plot(
        [min(energies_md), max(energies_md)],
        [min(energies_md), max(energies_md)],
        color="red",
        linestyle="--",
    )
    ...
    system.add_data(
        ...
        TensorMap(
            ...
            blocks=[
                TensorBlock(
                    values=torch.tensor(
                        atoms.get_momenta(), dtype=torch.float32, device=device
                    ).unsqueeze(-1),
                    samples=Labels(
                        names=["system", "atom"],
                        values=torch.tensor(
    ...
    system.add_data(
        ...
        TensorMap(
            ...
            blocks=[
                TensorBlock(
                    values=torch.tensor(
                        atoms.get_masses(), dtype=torch.float32, device=device
                    ).unsqueeze(-1),
                    samples=Labels(
                        names=["system", "atom"],
                        values=torch.tensor(
